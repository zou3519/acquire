...
class ScoreSheet(state: AcquireState) extends Actor {
  ...
  private def playerRowView(player: Int): (Seq[Int] => Seq[String]) =
    (corps: Seq[Int]) => Seq(state.config.playerName(player)) ++ corps.map {
      corp => state.sheet.shares(corp,player).toString
    } ++ Seq((state.sheet.cash(player)/100).toString, (state.sheet.netWorth(player)/100).toString)

  private val viewFns: Vector[Seq[Int] => Seq[String]] =
    (for(player <- 0 until 4) yield playerRowView(player)).toVector ++ Vector(
    rowView("size", state.sheet.chainSize),
    rowView("bank", corp => Some(state.sheet.bankShares(corp))),
    rowView("price/100", state.sheet.sharePrice(_).map(_/100)),
    rowView("major/100", state.sheet.firstBonus(_).map(_/100)),
    rowView("minor/100", state.sheet.secondBonus(_).map(_/100))
  )

  private def rows: Seq[Seq[String]] = viewFns.map(fn => fn(state.config.corps))
